document.addEventListener("DOMContentLoaded", () => {
  ...
  const defaultContacts = [
    { nome: "Atendimento Grão 1000", sub: "Atendimento nacional", fone: "(45) 99834-1000" }
  ];
        ...
        return `
          <div class="contactItem">
            ...
            <div class="contactBody">
              ...
              <div class="contactCity">${sub}</div>
              <a class="contactPhone" target="_blank" rel="noopener" href="${toWhatsappHref(phone)}">Entrar em contato</a>
            </div>
          </div>
        `;
      }).join("");
  ...
  const activateState = (states, current) => {
    ...
    const uf = String(current.dataset.uf || "").toUpperCase();
    renderContacts(uf);
  };
  ...
  const loadMap = async () => {
    ...
    const defaultUf =
      ...
      mapMount.querySelector('[data-uf="MT"]') ||
      states[0];

    if (defaultUf) {
      activateState(states, defaultUf);
    }
  };
